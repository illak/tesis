...
object DiscogsArtistSearch {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession
      .builder()
      .appName("Wikidata artist finder")
      .getOrCreate()
    ...
    // wikidata
    val parquet_dir = (if (args(1).last != '/') args(1).concat("/") else args(1)) + "wikidata.pqt"
    //=================================================================================
    ...
    def setUrl(nameCol: String): Column = {
      ...
      val baseurlwiki = "https://www.wikidata.org/wiki/"
      when(col(nameCol).isNull, col(nameCol))
            .otherwise(concat(lit(baseurlwiki), col(nameCol)))
    }
    ...
    for( name <- artNames ){
        var dfArtAux = dfArt2.filter($"name_lc".rlike(name))
                                .withColumn("wikidata_url", setUrl("wikidata_id"))
                                .select($"name",$"wikidata_url", $"wikidata_id")
